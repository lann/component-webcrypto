...
(jco's async ABI uses JSPI). The
[lann/component-test](https://github.com/lann/component-test) stack is a
git dependency pinned by rev in the root `Cargo.toml`, fetched by cargo on
first build — no local checkout needed. `./scripts/setup.sh` installs the
rest.
