...
//
// Usage: node compare.mjs <baseline.json> <roundtrip.json> [--update]

import { readFileSync, writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";

const LOSSES_PATH = fileURLToPath(new URL("./losses.js", import.meta.url));

const [baselinePath, roundtripPath, flag] = process.argv.slice(2);
if (!baselinePath || !roundtripPath || (flag !== undefined && flag !== "--update")) {
  console.error("usage: node compare.mjs <baseline.json> <roundtrip.json> [--update]");
  process.exit(2);
}
...
if (update) {
  writeLosses(losses.map((loss) => loss.key));
  console.log(
...
if (stale.length > 0) {
  problems.push(
    `${stale.length} recorded loss(es) were not observed (fixed, renamed, or no longer ` +
      `platform-passed) — re-record with \`just update-wpt-parity\` once the change is understood:\n` +
...
`;
  const body = keys
    ...
    .join("\n");
  writeFileSync(LOSSES_PATH, `${header}export const KNOWN_LOSSES = [\n${body}\n];\n`);
}
