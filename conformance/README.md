...
## Results-schema tolerance (ratified)
...
failures. This is not a gating change: the fold diverts an
unknown-status row out of the parsed results, the case is then missing
from coverage, and the aggregate's coverage check fails the run. Since
the lockfiles pin every generated leaf (lann/component-test#49's
`lock --leaves`; enforced by `lock-check` and regenerated by
`lock-update`, both of which enumerate the built suites), coverage is
exact-set-equality for the *entire* census — probes and generated rows
alike — so an unknown status cannot pass silently anywhere in the
corpus. An earlier ratification (#301) claimed this before it was true
for generated rows and was withdrawn (#302/#303); leaf pinning is what
makes it true. Ratified on that basis.
