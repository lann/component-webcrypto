...
//! per-case results as JSON — or, in `--lock-out` mode, writes the corpus
...
mod harness {
    ...
    /// corpus for a target missing `missing`, and drive each case whose
    ...
    pub async fn run_guest(
        ...
        // Materialize and enumerate the corpus (sync exports, store-style
...
fn parse_args() -> anyhow::Result<Args> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--corpus" => corpus = Some(args.next().context("--corpus needs a value")?),
    ...
    if args.out.is_some() && args.corpus.is_none() {
        anyhow::bail!("--out requires --corpus <shared|signing>");
    }
    if args.out.is_none() && args.lock_out.is_none() {
        anyhow::bail!("nothing to do: pass --out (with --corpus) and/or --lock-out");
    }
    Ok(args)
}

/// Render the lockfile: TOML, one inline table per case (name, then any
/// feature tags), in corpus order.
fn render_lock(entries: &[LockEntry]) -> String {
    let mut lock = String::from(
        "# The conformance corpus: one case per line (its name, plus the feature tags\n\
         # it exercises), in corpus order. Generated by `just update-conformance-lock`;\n\
         # do not edit. The runner validates every results file against this\n\
         # inventory, so corpus changes must land here intentionally.\n\
         cases = [\n",
    );
