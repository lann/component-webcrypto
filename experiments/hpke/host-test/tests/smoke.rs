...
const INFO: &[u8] = &hexlower!("4f6465206f6e2061204772656369616e2055726e");
const PT: &[u8] = &hexlower!("4265617574792069732074727574682c20747275746820626561757479");
const AAD: &[u8] = &hexlower!("436f756e742d30");

async fn known_answer(vector: Rfc9180Vector) {
    let component = component();

    let receiver = derive_key_pair(component, vector.ikm_r)
        .await
        .expect("host/guest failure")
        .expect("derive-key-pair(ikmR) failed");
    assert_eq!(receiver.secret_key, vector.sk_rm, "skRm");
    assert_eq!(receiver.public_key, vector.pk_rm, "pkRm");

    let ephemeral = derive_key_pair(component, vector.ikm_e)
        .await
        .expect("host/guest failure")
        .expect("derive-key-pair(ikmE) failed");
    assert_eq!(ephemeral.secret_key, vector.sk_em, "skEm");
    assert_eq!(ephemeral.public_key, vector.pk_em, "pkEm");
    ...
    assert_eq!(sealed.enc, vector.pk_em, "enc");
    assert_eq!(sealed.ciphertext, vector.ct, "ct");
...
async fn rfc9180_a1_base() {
    known_answer(Rfc9180Vector {
        ...
        ikm_e: &hexlower!("7268600d403fce431561aef583ee1613527cff655c1343f29812e66706df3234"),
        pk_em: &hexlower!("37fda3567bdbd628e88668c3c8d7e97d1d1253b6d4ea6d44c150f741f1bf4431"),
        sk_em: &hexlower!("52c4a758a802cd8b936eceea314432798d5baf2d7e9235dc084ab1b9cfa2f736"),
        ikm_r: &hexlower!("6db9df30aa07dd42ee5e8181afdb977e538f5e1fec8a06223f33f7013e525037"),
        pk_rm: &hexlower!("3948cfe0ad1ddb695d780e59077195da6c56506b027329794ab02bca80815c4d"),
        sk_rm: &hexlower!("4612c550263fc8ad58375df3f557aac531d26850903e55a9f23f21d8534e8ac8"),
        ct: &hexlower!(
