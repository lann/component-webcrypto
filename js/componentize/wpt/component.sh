...
gen_suites() {
    ...
    echo 'export { run_test };' >> "$B"/group-aes-ctr.js
    ...
    echo 'function run_wrap_tests() {' >> "$B"/group-wrap-key.js
    cat "$V"/wrapKey_unwrapKey.https.any.js >> "$B"/group-wrap-key.js
    printf '}\nexport { run_wrap_tests };\n' >> "$B"/group-wrap-key.js
