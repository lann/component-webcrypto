// Tests for the RSA signing environment-default policy — the posture the
// conformance suite cannot observe: its jco targets run under Node, where
// the default is to serve, so the decline default (browsers, unknown
// runtimes) and the module-level opt-in are exercised here by masking
// `globalThis.process`. These run the host directly — no component, no
// jco — because the policy is reached through the same minting functions a
...
const resetPolicy = () => setRsaSigningPolicy(undefined);
after(resetPolicy);
...
const declined = (err) =>
  err.tag === "unsupported" && err.val.includes("setRsaSigningPolicy");
...
test("a non-Node environment declines every minting function by default", async () => {
...
test("the policy setter rejects unknown postures", () => {
  assert.throws(() => setRsaSigningPolicy("maybe"), TypeError);
  assert.throws(() => setRsaSigningPolicy(null), TypeError);
});
