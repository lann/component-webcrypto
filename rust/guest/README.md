...
Guest-side Rust library over the `polymorph:webcrypto` imports: the intended way
for Rust components to *consume* the package (the JS counterpart is
[`@polymorph/webcrypto-componentize`](../../js/componentize)). It binds the full
...
```rust
use polymorph_webcrypto_guest::{hmac_sha2, MacKeyOptions};
use polymorph_webcrypto_guest::hmac_sha2::Sha2Variant;
...
Most consumers need no `polymorph:webcrypto` WIT of their own: link this crate
...
the `ShortWrite` mapping (`cargo test -p polymorph-webcrypto-guest --all-features`).
