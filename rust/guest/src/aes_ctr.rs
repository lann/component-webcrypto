//! `aes-ctr` key creation (the unauthenticated AES-CTR mode; prefer
//! [`aes_gcm`](crate::aes_gcm) — see [`CipherKey`]'s warning).

use crate::{bindings, CipherKey, CipherKeyOptions, Error};

pub use crate::bindings::aes_ctr::AesVariant;
...
pub async fn import_key_jwk(
    ...
    Ok(CipherKey::from_raw(
        bindings::aes_ctr::import_key_jwk(variant, jwk.into(), options.lower()).await?,
    ))
}
